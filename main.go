...
func uploadTree(s3 *S3, dataPath string) error {
	...
	if err := s3.UploadDirectory(path.Join(dataPath, "metadata"), path.Join(config.S3.PrefixKey, "metadata")); err != nil {
		return fmt.Errorf("can't upload metadata: %v", err)
	}
	log.Printf("upload data")
	if err := s3.UploadDirectory(path.Join(dataPath, "shadow"), path.Join(config.S3.PrefixKey, "shadow")); err != nil {
...
func uploadArchive(s3 *S3, dataPath string) error {
	...
	log.Printf("upload data")
	if err := s3.UploadFile(file.Name(), path.Join(config.S3.PrefixKey, filepath.Base(file.Name()))); err != nil {
...
func downloadTree(s3 *S3, dataPath string) error {
	if err := s3.DownloadTree(path.Join(config.S3.PrefixKey, "metadata"), path.Join(dataPath, "backup", "metadata")); err != nil {
		return fmt.Errorf("cat't download metadata from s3 with %v", err)
	}
	if err := s3.DownloadTree(path.Join(config.S3.PrefixKey, "shadow"), path.Join(dataPath, "backup", "shadow")); err != nil {
...
func removeOldBackups(config Config, s3 *S3) error {
	objects, err := s3.ListObjects(config.S3.PrefixKey)
	if err != nil {
		return err
	}
